...
def make_file_spec_suffix(filter, recursive)
...
action :set do
  file_spec_suffix = make_file_spec_suffix(new_resource.filter, new_resource.recursive)
  ...
  if new_resource.filter == '*' || new_resource.recursive
    not_if = fcontext_defined(
